...
public class StreamDemoApp {
	...
	public static void main(String[] args) {
		...
		// map and reduce with Math.max - p. 721
		...
		//we didn't have to use map because our list just contains numbers, not invoice objects
		// we has List<Double> as opposed to List<Invoice>
		double maxInvoiceTotal = invoices.stream()
				.reduce(0.0, (a, b) -> Math.max(a, b));
		System.out.println("Max invoice total: "+ maxInvoiceTotal);
		
		//map and reduce with Math.max + method reference - p. 721
